...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [players, daily] = await Promise.all([
      prisma.player.findMany({select:{id:true,slug:true,name:true,title:true,team:true,playerNumber:true,imageUrl:true,eliminated:true,upvoteCount:true,downvoteCount:true}}),
    ...
    const resolve = async (p: typeof players[number]) => ({id:p.id,slug:p.slug,name:p.name,title:p.title,team:p.team,imageUrl:await resolvePlayerImageUrl(p.playerNumber,p.imageUrl),upvoteCount:p.upvoteCount});
